...
echo ============================================================
echo  Repair compatibility paths safely
echo ============================================================
...
rem Never create, replace, move, or delete D:\stock itself.
rem Only create the old D:\Downloads\stock junction when the old path is absent.
...
if not exist "%LEGACY_ROOT%" (
    ...
    if !ERRORLEVEL! NEQ 0 echo [WARN] Legacy junction creation failed. Administrator rights may be required.
) else (
...
echo.
echo [1/2] Creating old flat external_repos links...
...
echo.
echo [2/2] Creating early github_sources links...
...
echo.
echo ============================================================
echo  Compatibility path repair completed
echo ============================================================
echo Repository: %REPO_ROOT%
echo Existing real folders were not moved, deleted, or overwritten.
echo.
...
if not exist "%TARGET%" (
    echo [SKIP] Source is not downloaded: %~2
    exit /b 0
)

if exist "%LINK%" (
    echo [KEEP] Existing path was not modified: %~1
    exit /b 0
)
...
if %ERRORLEVEL% EQU 0 (
    ...
    echo [WARN] Junction creation failed: %~1
)
exit /b 0
